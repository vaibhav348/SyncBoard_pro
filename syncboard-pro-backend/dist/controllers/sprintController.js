...
exports.getSprintSummary = exports.completeSprint = exports.startSprint = exports.updateSprint = exports.getAllSprints = exports.createSprint = void 0;
...
const sprintAuthorization_1 = require("../utils/sprintAuthorization");
// --- ZOD SCHEMAS FOR VALIDATION ---
const CreateSprintSchema = zod_1.default.object({
    projectId: zod_1.default.string().min(1, "Project ID is required"),
    name: zod_1.default.string().min(2, "Name should be at least 2 characters long"),
    goal: zod_1.default.string().min(2, "Goal should be at least 2 characters long").optional(),
    startDate: zod_1.default.coerce.date().optional(),
    endDate: zod_1.default.coerce.date().optional()
});
const StartSprintSchema = zod_1.default.object({
    projectId: zod_1.default.string().min(1, "Project ID is required"),
    startDate: zod_1.default.coerce.date().optional(),
    endDate: zod_1.default.coerce.date().optional()
}).strict();
const CompleteSprintSchema = zod_1.default.object({
    moveIncompleteTo: zod_1.default.enum(["backlog", "sprint"]).default("backlog"),
    targetSprintId: zod_1.default.string().min(1).optional()
}).strict();
const UpdateSprintSchema = zod_1.default.object({
    name: zod_1.default.string().min(2, "Sprint name should be at least 2 characters long").optional(),
    goal: zod_1.default.string().optional(),
    startDate: zod_1.default.coerce.date().optional(),
    endDate: zod_1.default.coerce.date().optional()
}).strict();
...
const createSprint = async (req, res) => {
    try {
        ...
        const newSprint = await Sprint_1.default.create({
            projectId,
            name,
            goal,
            startDate,
...
const getAllSprints = async (req, res) => {
    try {
        ...
        const sprints = await Sprint_1.default.find({ projectId }).sort({ createdAt: -1 });
...
const updateSprint = async (req, res) => {
    try {
        ...
        const project = await Project_1.default.findById(sprint.projectId);
        if (!project) {
            return res.status(404).json({ message: "Project not found" });
        }
        const projectLike = project ? { ...project.toObject(), _id: String(project._id) } : null;
        const sprintLike = sprint ? { ...sprint.toObject(), _id: String(sprint._id) } : null;
        if (!(0, sprintAuthorization_1.canManageSprint)(user, projectLike, sprintLike)) {
...
const startSprint = async (req, res) => {
    try {
        ...
        const existingActive = await Sprint_1.default.findOne({ projectId, status: "active", _id: { $ne: sprintId } });
        if (existingActive) {
...
const completeSprint = async (req, res) => {
    try {
        ...
        if (moveIncompleteTo === "sprint") {
            ...
            const targetSprint = await Sprint_1.default.findById(targetSprintId);
...
const getSprintSummary = async (req, res) => {
    try {
        ...
        const [tasks, issues, stories] = await Promise.all([
            Task_1.default.find({ sprintId: sprint._id }),
            Issue_1.default.find({ sprintId: sprint._id }),
            UserStory_1.default.find({ sprintId: sprint._id })
        ]);
        ...
        const completedStoryPoints = stories.filter((story) => ["Done", "done", "Completed", "completed", "Closed", "closed"].includes(String(story.status ?? ""))).reduce((sum, story) => sum + (story.storyPoints ?? 0), 0);
        return res.status(200).json({
            sprint,
            counts: {
                tasks: tasks.length,
...
exports.getSprintSummary = getSprintSummary;
