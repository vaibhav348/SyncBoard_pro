...
const IssueSchema = z.object({
    ...
    type: z.enum(['Bug', 'Feature', 'Issue'], { message: "Type is mandatory" }),
    severity: z.enum(['Low', 'Normal', 'High', 'Critical']).default('Normal'),
    projectId: z.string().min(1, "ProjectId is mandatory"),
    assignedTo: z.string().optional(),
    taskId: z.string().optional(),      // <-- Add kiya
...
const FullUpdateIssueSchema = z.object({
    title: z.string().min(2, "Title should be at least 2 characters long").optional(),
    description: z.string().min(10, "Description should be at least 10 characters long").optional(),
    status: z.enum(['Todo', 'In-Progress', 'Review', 'Done']).optional(),
    priority: z.enum(['Low', 'Medium', 'High']).optional(),
    type: z.enum(['Bug', 'Feature', 'Issue']).optional(),        // 👈 add
    severity: z.enum(['Low', 'Normal', 'High', 'Critical']).optional(), // 👈 add
    assignedTo: z.string().optional()
});
...
export const updateIssue = async (req: Request, res: Response) => {
    try {
        ...
        const issue = await Issue.findOne({ _id: issueId, companyId });
        if (!issue) {
        ...
        const isCreator = Boolean(
            (issue as any).createdBy?.toString() === userId ||
            (issue as any).assignedBy?.toString() === userId
        );
...
export const getIssueById = async (req: Request, res: Response) => {
    try {
        ...
        if (!mongoose.Types.ObjectId.isValid(issueId as string)) {
...
export const addCommentToIssue = async (req: Request, res: Response) => {
    try {
        const { issueId } = req.params;
        ...
        const newComment = new Comment({
            issueId: issueId as string, // Explicitly string cast kiya safe rehne ke liye
            userId: user.id,
            content
        });
        ...
        const populatedComment = await Comment.findById(newComment._id).populate('userId', 'name email profilePic');
...
export const updateIssueComment = async (req: Request, res: Response) => {
    try {
        const user = (req as any).user;
...
export const getIssueComments = async (req: Request, res: Response) => {
    try {
        ...
        const comments = await Comment.find({ issueId: issueId as string })
...
export const getIssuesByTask = async (req: Request, res: Response) => {
    try {
        ...
        const issues = await Issue.find({ taskId: new mongoose.Types.ObjectId(taskId) })
            .populate("assignedTo", "name email")
            .sort({ createdAt: -1 });
...
// GET ISSUES LINKED TO A SPECIFIC TASK
export const getIssuesBySprint = async (req: Request, res: Response) => {
    try {
        const sprintId = req.params.sprintId as string;
        if (!sprintId) return res.status(400).json({ message: "Task ID is required" });

        const issues = await Issue.find({ sprintId: new mongoose.Types.ObjectId(sprintId) })
            .populate("assignedTo", "name email")
            .sort({ createdAt: -1 });
